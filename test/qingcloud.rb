...
def invoke_scenario(subservice)
  eval <<-SCENARIO
    ...
    When(/^initialize #{subservice} service$/) do
      #{subservice}_service = @shanhe_service.instance @test_config[:zone]
    end
...
When(/^initialize ShanHe service$/) do
  init_shanhe_service
end

Then(/^the ShanHe service is initialized$/) do
  fail if @shanhe_service == nil
end

# ----------------------------------------------------------------------------

When(/^describe zones$/) do
  @describe_zones_output = @shanhe_service.describe_zones
end
